class DrawingBezier extends PaintFunction { //using one canvas draft only
    ...
    onDragging(coord, event) {
        if (!this.finish) {
            ...
            if (!this.phase_adjust) {
                //mid at beginning is: 
                this.midpt1 = {
                    x: (this.quadcoord.start.x + (coord[0] - this.quadcoord.start.x) / 4),
                    y: (this.quadcoord.start.y + (coord[1] - this.quadcoord.start.y) / 4) - 40
                };
                this.midpt2 = {
                    x: (this.quadcoord.start.x + (coord[0] - this.quadcoord.start.x) / 4 * 3),
                    y: (this.quadcoord.start.y + (coord[1] - this.quadcoord.start.y) / 4) + 40
                };
                this.endpt = {
                    x: coord[0],
    ...
    onMouseLeave() {
        ...
        if (!this.phase_adjust) {
            ...
            this.midpt1 = {
                x: (this.quadcoord.start.x + (this.prevCoord.x - this.quadcoord.start.x) / 4),
                y: (this.quadcoord.start.y + (this.prevCoord.y - this.quadcoord.start.y) / 4) - 40
            };
            this.midpt2 = {
                x: (this.quadcoord.start.x + (this.prevCoord.x - this.quadcoord.start.x) / 4 * 3),
                y: (this.quadcoord.start.y + (this.prevCoord.y - this.quadcoord.start.y) / 4) + 40
            };
    ...
    drawCPDraft(x, y) {
        ...
        this.contextDraft.strokeStyle = this.contextReal.strokeStyle = "black";
        this.contextDraft.setLineDash([1, 3])
